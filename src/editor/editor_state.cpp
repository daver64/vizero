...
vizero_editor_state_t* vizero_editor_state_create(void) {
    ...
    if (state) {
        ...
        /* Initialize smart indentation settings */
        state->use_tabs = 0;        /* Default to spaces */
        state->tabstop = 4;         /* Default tab width */
        state->auto_indent = 1;     /* Enable auto-indent by default */
        state->smart_indent = 1;    /* Enable smart indent by default */
    }
    return state;
}
...
vizero_editor_state_t* vizero_editor_state_create_with_settings(vizero_settings_t* settings) {
    ...
    if (state) {
        ...
        /* Initialize code folding - will be set up when buffers are created */
        state->code_folding = NULL;
        ...
        if (state->buffers[0] && state->cursors[0]) {
            ...
            /* Initialize code folding with the first buffer */
...
void vizero_editor_state_destroy(vizero_editor_state_t* state) {
    ...
    /* Clean up code folding */
    if (state->code_folding) {
        vizero_code_folding_destroy(state->code_folding);
    }
...
int vizero_editor_execute_command(vizero_editor_state_t* state, const char* command) {
    ...
    } else if (strcmp(command, "za") == 0) {
        /* Toggle fold at cursor */
        return vizero_editor_toggle_fold_at_cursor(state);
        
    } else if (strcmp(command, "zc") == 0) {
        /* Close fold at cursor */
        return vizero_editor_close_fold_at_cursor(state);
        
    } else if (strcmp(command, "zo") == 0) {
        /* Open fold at cursor */
        return vizero_editor_open_fold_at_cursor(state);
        
    } else if (strcmp(command, "zf") == 0) {
        /* Create fold at cursor */
        return vizero_editor_create_fold_at_cursor(state);
        
    } else if (strcmp(command, "zd") == 0) {
        /* Delete fold at cursor */
        return vizero_editor_delete_fold_at_cursor(state);
        
    } else if (strcmp(command, "zR") == 0) {
        /* Open all folds */
        return vizero_editor_open_all_folds(state);
        
    } else if (strcmp(command, "zM") == 0) {
        /* Close all folds */
        return vizero_editor_close_all_folds(state);
        
    /* SMART INDENTATION COMMANDS */
    } else if (strcmp(command, "indent") == 0 || strcmp(command, "==") == 0) {
        /* Auto-indent current line */
        return vizero_editor_auto_indent_line(state);
        
    } else if (strcmp(command, "indent-to-end") == 0 || strcmp(command, "=G") == 0) {
        /* Auto-indent from cursor to end */
        return vizero_editor_auto_indent_to_end(state);
        
    } else if (strcmp(command, "indent-all") == 0 || strcmp(command, "gg=G") == 0) {
        /* Auto-indent entire file */
        return vizero_editor_auto_indent_entire_file(state);
        ...
        *dest = '\0';
    ...
    } else if (strcmp(command, "retab") == 0) {
        /* Convert tabs to spaces or vice versa */
        if (!state->window_manager) return -1;
        ...
        /* Simple retab implementation - convert all tabs to spaces */
        int tabstop = vizero_editor_get_tabstop(state);
        ...
        for (size_t i = 0; i < line_count; i++) {
            ...
            while (*src && (size_t)(dst - new_line) < sizeof(new_line) - 8) {
                if (*src == '\t') {
                    /* Replace tab with appropriate number of spaces */
                    int spaces_to_add = tabstop - ((dst - new_line) % tabstop);
                    for (int j = 0; j < spaces_to_add && (size_t)(dst - new_line) < sizeof(new_line) - 1; j++) {
                        *dst++ = ' ';
                    }
                    src++;
                } else {
            ...
            *dst = '\0';
            ...
            /* Replace the line in the buffer by deleting and inserting */
            vizero_buffer_delete_line(buffer, i);
            vizero_buffer_insert_line(buffer, i, new_line);
            changed_lines++;
        }
        
        char msg[128];
        snprintf(msg, sizeof(msg), "Retabbed %d lines", changed_lines);
        vizero_editor_set_status_message(state, msg);
        return 0;
        
    } else if (strncmp(command, "set expandtab", 13) == 0) {
        /* Enable space indentation */
        state->use_tabs = 0;
        vizero_editor_set_status_message(state, "Using spaces for indentation");
        return 0;
        
    } else if (strncmp(command, "set noexpandtab", 15) == 0) {
        /* Enable tab indentation */
        state->use_tabs = 1;
        vizero_editor_set_status_message(state, "Using tabs for indentation");
        return 0;
        
    } else if (strncmp(command, "set tabstop", 11) == 0) {
        /* Set tab stop width */
        const char* value_str = command + 11;
        ...
        if (value > 0 && value <= 16) {
            state->tabstop = value;
            char msg[64];
            snprintf(msg, sizeof(msg), "Tab stop set to %d", value);
            vizero_editor_set_status_message(state, msg);
            return 0;
        } else {
            vizero_editor_set_status_message(state, "Invalid tab stop value (1-16)");
            return -1;
        }
...
int vizero_editor_paste_at_cursor(vizero_editor_state_t* state) {
    ...
    /* Get text from system clipboard first and make a safe local copy */
    char* clipboard_text = SDL_GetClipboardText();
    char* local_clipboard_copy = NULL;
    const char* content_to_paste = NULL;
    
    if (clipboard_text && strlen(clipboard_text) > 0) {
        /* Make a safe local copy to avoid platform-specific SDL memory issues */
        size_t text_len = strlen(clipboard_text);
        local_clipboard_copy = (char*)malloc(text_len + 1);
        if (local_clipboard_copy) {
            memcpy(local_clipboard_copy, clipboard_text, text_len);
            local_clipboard_copy[text_len] = '\0';
            content_to_paste = local_clipboard_copy;
        }
        /* clipboard_text already freed */;
        clipboard_text = NULL;
    } else {
    ...
    /* Additional safety check for content_to_paste */
    if (!content_to_paste || strlen(content_to_paste) == 0) {
        if (local_clipboard_copy) free(local_clipboard_copy);
        return -1;
    }
    ...
    if (newline_pos != NULL || carriage_return_pos != NULL) {
        ...
        *dst = '\0';
        ...
        /* First ensure we have a valid position after any buffer modifications */
        size_t buffer_line_count = vizero_buffer_get_line_count(buffer);
        if (buffer_line_count == 0) {
            /* Create empty buffer with one line if needed */
            vizero_buffer_insert_line(buffer, 0, "");
            buffer_line_count = 1;
            pos.line = 0;
            pos.column = 0;
        ...
        if (pos.line >= buffer_line_count) {
            pos.line = buffer_line_count - 1;
        }
        
        const char* current_line = vizero_buffer_get_line_text(buffer, pos.line);
        if (!current_line) {
            /* If we still can't get the current line, clean up and fail */
            for (size_t i = 0; i < line_count; i++) {
        ...
        if (line_count > 0) {
            ...
            size_t safe_column = (pos.column <= current_line_len) ? pos.column : current_line_len;
            
            /* Split current line: part before cursor + part after cursor */
            char* line_before = (char*)malloc(safe_column + 1);
            char* line_after = (char*)malloc(current_line_len - safe_column + 1);
            
            if (line_before && line_after) {
                strncpy(line_before, current_line, safe_column);
                line_before[safe_column] = '\0';
                strcpy(line_after, current_line + safe_column);
                
                /* Create first line: line_before + first_paste_line */
                /* Check if first line exists before using it */
                if (!lines[0]) {
                    free(line_before);
                    free(line_after);
                    for (size_t i = 0; i < line_count; i++) {
        ...
        /* Ensure position is still valid for single-line paste */
        size_t buffer_line_count = vizero_buffer_get_line_count(buffer);
        if (buffer_line_count == 0) {
            vizero_buffer_insert_line(buffer, 0, "");
            pos.line = 0;
            pos.column = 0;
        } else if (pos.line >= buffer_line_count) {
            pos.line = buffer_line_count - 1;
        }
        
        /* Validate column position within the line */
        const char* current_line = vizero_buffer_get_line_text(buffer, pos.line);
        if (current_line) {
...
int vizero_editor_toggle_command_palette(vizero_editor_state_t* state) {
    ...
    /* Enhanced implementation - show commands in a popup with timeout */
    const char* command_help = 
        "VIZERO COMMAND REFERENCE\n\n"
        "== CODE FOLDING ==\n"
        ...
        ":wq    - Write and quit\n\n"
        "== OTHER ==\n"
        "F1 or Ctrl+Shift+P - Show this help\n"
        "ESC    - Return to normal mode\n\n"
        "Press ESC to close this help.";
    
    /* Show as popup with 10 second timeout */
    vizero_editor_show_popup(state, command_help, 10000);
    return 0;
}
...
int vizero_editor_toggle_fold_at_cursor(vizero_editor_state_t* state) {
    if (!state || !state->code_folding) return -1;
    
    /* Simple implementation - find matching braces */
    vizero_buffer_t* buffer = vizero_editor_get_current_buffer(state);
    vizero_cursor_t* cursor = vizero_editor_get_current_cursor(state);
    if (!buffer || !cursor) return -1;
    ...
    /* First, check if there's an existing fold that starts exactly at cursor line */
    for (size_t i = 0; i < state->code_folding->fold_count; i++) {
        vizero_code_fold_t* fold = &state->code_folding->folds[i];
        if (fold->start_line == cursor_line) {
            /* Toggle existing fold that starts at cursor */
            fold->is_folded = !fold->is_folded;
            char msg[256];
            snprintf(msg, sizeof(msg), "Fold lines %zu-%zu %s", 
                     fold->start_line + 1, fold->end_line + 1,
                     fold->is_folded ? "closed" : "opened");
            vizero_editor_set_status_message(state, msg);
            return 0;
        }
    }
    
    /* Check if cursor is inside a folded region - if so, unfold the containing fold */
    if (vizero_code_folding_is_line_folded(state->code_folding, cursor_line)) {
        for (size_t i = 0; i < state->code_folding->fold_count; i++) {
            vizero_code_fold_t* fold = &state->code_folding->folds[i];
            if (cursor_line >= fold->start_line && cursor_line <= fold->end_line && fold->is_folded) {
                fold->is_folded = 0;  /* Unfold */
                char msg[256];
                snprintf(msg, sizeof(msg), "Opened fold lines %zu-%zu", 
                         fold->start_line + 1, fold->end_line + 1);
                vizero_editor_set_status_message(state, msg);
                return 0;
            }
    ...
    /* Look for opening brace on current line first, then check nearby lines */
    const char* current_line = vizero_buffer_get_line_text(buffer, cursor_line);
    size_t fold_start = cursor_line;
    int found_opening = 0;
    
    /* Check current line first */
    if (current_line && strstr(current_line, "{")) {
        fold_start = cursor_line;
        found_opening = 1;
    } else {
        /* Look backwards only a few lines (max 3) to avoid folding distant blocks */
        size_t search_limit = cursor_line > 3 ? cursor_line - 3 : 0;
        for (size_t i = cursor_line; i >= search_limit && !found_opening; i--) {
    ...
    if (!found_opening) {
        vizero_editor_set_status_message(state, "No foldable block found");
        return -1;
    }
    
    /* Find matching closing brace */
    int brace_count = 0;
    size_t fold_end = fold_start;
    
    for (size_t i = fold_start; i < line_count; i++) {
        const char* line_text = vizero_buffer_get_line_text(buffer, i);
        if (line_text) {
            for (const char* c = line_text; *c; c++) {
                if (*c == '{') brace_count++;
                else if (*c == '}') {
                    brace_count--;
                    if (brace_count == 0) {
...
found_end:
    if (brace_count != 0) {
        vizero_editor_set_status_message(state, "No matching closing brace found");
        return -1;
    }
    
    if (fold_end <= fold_start + 1) {
        vizero_editor_set_status_message(state, "Block too small to fold");
        return -1;
    }
    
    /* Check if this range is already folded */
    for (size_t i = 0; i < state->code_folding->fold_count; i++) {
        vizero_code_fold_t* fold = &state->code_folding->folds[i];
        if (fold->start_line == fold_start && fold->end_line == fold_end) {
    ...
    /* Create new fold and close it */
    if (vizero_code_folding_fold_range(state->code_folding, fold_start, fold_end) == 0) {
        char msg[256];
        snprintf(msg, sizeof(msg), "Folded lines %zu-%zu (%zu lines)", 
                 fold_start + 1, fold_end + 1, fold_end - fold_start + 1);
        vizero_editor_set_status_message(state, msg);
        return 0;
    } else {
        vizero_editor_set_status_message(state, "Failed to create fold");
        return -1;
    }
}
...
int vizero_editor_open_fold_at_cursor(vizero_editor_state_t* state) {
    ...
    /* Find and open any fold containing the cursor line */
    for (size_t i = 0; i < state->code_folding->fold_count; i++) {
        vizero_code_fold_t* fold = &state->code_folding->folds[i];
        if (cursor_line >= fold->start_line && cursor_line <= fold->end_line && fold->is_folded) {
            fold->is_folded = 0;  /* Open the fold */
            char msg[256];
            snprintf(msg, sizeof(msg), "Opened fold at lines %zu-%zu", 
                     fold->start_line + 1, fold->end_line + 1);
            vizero_editor_set_status_message(state, msg);
            return 0;
        }
    ...
    vizero_editor_set_status_message(state, "No folded block found at cursor");
    return -1;
}
...
int vizero_editor_close_fold_at_cursor(vizero_editor_state_t* state) {
    ...
    /* First try to find an existing fold at cursor and close it */
    for (size_t i = 0; i < state->code_folding->fold_count; i++) {
        vizero_code_fold_t* fold = &state->code_folding->folds[i];
        if (cursor_line >= fold->start_line && cursor_line <= fold->end_line && !fold->is_folded) {
            fold->is_folded = 1;  /* Close the fold */
            char msg[256];
            snprintf(msg, sizeof(msg), "Closed fold at lines %zu-%zu", 
                     fold->start_line + 1, fold->end_line + 1);
            vizero_editor_set_status_message(state, msg);
            return 0;
        }
    ...
    /* If no existing fold, try to create one like za does */
    return vizero_editor_toggle_fold_at_cursor(state);
}
...
int vizero_editor_create_fold_at_cursor(vizero_editor_state_t* state) {
    ...
    /* Same logic as toggle but always creates a new fold */
    vizero_buffer_t* buffer = vizero_editor_get_current_buffer(state);
    vizero_cursor_t* cursor = vizero_editor_get_current_cursor(state);
    if (!buffer || !cursor) return -1;
    ...
    /* Look for opening brace on current line or lines above */
    size_t fold_start = cursor_line;
    int found_opening = 0;
...
found_end:
    if (brace_count != 0 || fold_end <= fold_start + 1) {
        vizero_editor_set_status_message(state, "Invalid foldable block");
        return -1;
    }
    
    /* Create new fold (always unfold initially) */
    if (vizero_code_folding_unfold_range(state->code_folding, fold_start, fold_end) == 0) {
        char msg[256];
        snprintf(msg, sizeof(msg), "Created fold for lines %zu-%zu", 
                 fold_start + 1, fold_end + 1);
        vizero_editor_set_status_message(state, msg);
        return 0;
    } else {
...
int vizero_editor_delete_fold_at_cursor(vizero_editor_state_t* state) {
    ...
    /* Find and remove any fold containing the cursor line */
    for (size_t i = 0; i < state->code_folding->fold_count; i++) {
        vizero_code_fold_t* fold = &state->code_folding->folds[i];
        if (cursor_line >= fold->start_line && cursor_line <= fold->end_line) {
            /* Remove this fold by shifting remaining folds down */
            for (size_t j = i; j < state->code_folding->fold_count - 1; j++) {
                state->code_folding->folds[j] = state->code_folding->folds[j + 1];
            }
            state->code_folding->fold_count--;
            
            char msg[256];
            snprintf(msg, sizeof(msg), "Deleted fold at lines %zu-%zu", 
                     fold->start_line + 1, fold->end_line + 1);
            vizero_editor_set_status_message(state, msg);
            return 0;
        }
    ...
    vizero_editor_set_status_message(state, "No fold found at cursor");
    return -1;
}
...
int vizero_editor_open_all_folds(vizero_editor_state_t* state) {
    ...
    /* Open all existing folds */
    for (size_t i = 0; i < state->code_folding->fold_count; i++) {
        if (state->code_folding->folds[i].is_folded) {
            state->code_folding->folds[i].is_folded = 0;
            opened_count++;
        }
    }
    
    char msg[256];
    snprintf(msg, sizeof(msg), "Opened %zu folds", opened_count);
    vizero_editor_set_status_message(state, msg);
...
int vizero_editor_close_all_folds(vizero_editor_state_t* state) {
    ...
    for (size_t i = 0; i < state->code_folding->fold_count; i++) {
        if (!state->code_folding->folds[i].is_folded) {
            state->code_folding->folds[i].is_folded = 1;
            closed_count++;
        }
    }
    
    char msg[256];
    snprintf(msg, sizeof(msg), "Closed %zu folds", closed_count);
    vizero_editor_set_status_message(state, msg);
...
int vizero_editor_auto_indent_line(vizero_editor_state_t* state) {
    ...
    /* Look at previous non-empty line to determine base indentation */
    for (size_t i = cursor_line; i > 0; i--) {
        const char* prev_line = vizero_buffer_get_line_text(buffer, i - 1);
        if (prev_line && strlen(prev_line) > 0) {
            /* Count leading spaces/tabs */
            size_t spaces = 0;
            for (const char* c = prev_line; *c && (*c == ' ' || *c == '\t'); c++) {
                if (*c == ' ') spaces++;
                else if (*c == '\t') spaces += state->tabstop; /* Use tabstop setting */
            }
            indent_level = spaces;
    ...
    /* Add indentation */
    if (state->use_tabs) {
        size_t tabs = indent_level / state->tabstop;
        size_t spaces = indent_level % state->tabstop;
        for (size_t i = 0; i < tabs && pos < sizeof(new_line) - 1; i++) {
            new_line[pos++] = '\t';
        }
        for (size_t i = 0; i < spaces && pos < sizeof(new_line) - 1; i++) {
            new_line[pos++] = ' ';
        }
    } else {
        for (size_t i = 0; i < indent_level && pos < sizeof(new_line) - 1; i++) {
            new_line[pos++] = ' ';
        }
    }
    
    /* Add non-whitespace content from original line */
    for (const char* c = trimmed; *c && pos < sizeof(new_line) - 1; c++) {
        new_line[pos++] = *c;
    }
    new_line[pos] = '\0';
    
    /* Replace the line */
    vizero_buffer_delete_line(buffer, cursor_line);
    vizero_buffer_insert_line(buffer, cursor_line, new_line);
    
    vizero_editor_set_status_message(state, "Line auto-indented");
    return 0;
}
...
int vizero_editor_auto_indent_to_end(vizero_editor_state_t* state) {
    ...
    /* Auto-indent from cursor line to end of buffer */
    for (size_t i = cursor_line; i < line_count; i++) {
        /* Set cursor to this line temporarily */
        vizero_cursor_set_position(cursor, i, 0);
        
        /* Auto-indent this line */
        if (vizero_editor_auto_indent_line(state) == 0) {
            lines_processed++;
        }
    }
    
    /* Restore cursor to original line */
    vizero_cursor_set_position(cursor, cursor_line, 0);
    
    char msg[128];
    snprintf(msg, sizeof(msg), "Auto-indented %zu lines to end", lines_processed);
    vizero_editor_set_status_message(state, msg);
    return 0;
}
...
int vizero_editor_auto_indent_entire_file(vizero_editor_state_t* state) {
    ...
    char msg[128];
    snprintf(msg, sizeof(msg), "Auto-indented entire file (%zu lines)", lines_processed);
    vizero_editor_set_status_message(state, msg);
    return 0;
}

/* Fold-aware cursor movement functions */
void vizero_editor_cursor_move_down_fold_aware(vizero_editor_state_t* state) {
    ...
    size_t current_line = vizero_cursor_get_line(cursor);
    size_t line_count = vizero_buffer_get_line_count(vizero_editor_get_current_buffer(state));
    ...
    /* If we landed in a folded region, skip to the end of the fold */
    if (state->code_folding && vizero_code_folding_is_line_folded(state->code_folding, new_line)) {
        for (size_t i = 0; i < state->code_folding->fold_count; i++) {
            vizero_code_fold_t* fold = &state->code_folding->folds[i];
            if (new_line >= fold->start_line && new_line <= fold->end_line && fold->is_folded) {
                /* Move cursor to the line after the fold */
                size_t target_line = fold->end_line + 1;
                if (target_line < line_count) {
                    vizero_cursor_set_position(cursor, target_line, 0);
                    /* Clamp column to new line length */
                    vizero_buffer_t* buffer = vizero_editor_get_current_buffer(state);
                    if (buffer) {
                        size_t line_len = vizero_buffer_get_line_length(buffer, target_line);
                        if (vizero_cursor_get_column(cursor) > line_len) {
                            vizero_cursor_set_position(cursor, target_line, line_len);
                        }
                    }
                }
                break;
...
void vizero_editor_cursor_move_up_fold_aware(vizero_editor_state_t* state) {
    ...
    /* If we landed in a folded region, skip to the start of the fold */
    if (state->code_folding && vizero_code_folding_is_line_folded(state->code_folding, new_line)) {
        for (size_t i = 0; i < state->code_folding->fold_count; i++) {
            vizero_code_fold_t* fold = &state->code_folding->folds[i];
            if (new_line >= fold->start_line && new_line <= fold->end_line && fold->is_folded) {
                /* Move cursor to the line before the fold */
                if (fold->start_line > 0) {
                    size_t target_line = fold->start_line - 1;
                    vizero_cursor_set_position(cursor, target_line, 0);
                    /* Clamp column to new line length */
                    vizero_buffer_t* buffer = vizero_editor_get_current_buffer(state);
                    if (buffer) {
                    ...
                    /* If fold starts at line 0, stay at line 0 */
                    vizero_cursor_set_position(cursor, 0, 0);
                }
                break;
            }
