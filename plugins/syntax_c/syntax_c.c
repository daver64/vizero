/* C/C++/Assembly syntax highlighting plugin */
...
static int get_file_type(const char* filename) {
    ...
    return 0; /* Not handled by this plugin */
}
...
static int highlight_syntax(vizero_buffer_t* buffer, size_t start_line, size_t end_line,
    ...
    /* Only handle C/C++/Assembly files */
    ...
    for (size_t line = start_line; line <= end_line && count < max_tokens; line++) {
        ...
        int n = 0;
        if (file_type == 2) {
            n = highlight_asm_line(line_text, line, tokens + count, max_tokens - count);
        } else if (file_type == 1) {
            n = highlight_c_line(line_text, line, tokens + count, max_tokens - count);
        }
        if (n > 0) count += n;
    }
    *token_count = count;
